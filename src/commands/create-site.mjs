...
- 如果需要登录，说明登录页已打开，并等待用户下次确认后再执行 \`complete-login\` 和重试。
...
function looksLikeLoginPage(html) {
  ...
  return (
    /登录|login|sign\\s*in/i.test(title) ||
    (/登录|login|sign\\s*in/i.test(text) && /用户名|密码|password|remember me/i.test(text))
  );
}

export async function fetchSiteContent(input) {
  const site = input.site || "${site}";
  await ensureAuthenticated(site, input.url);
  ...
  try {
    ...
    if ([401, 403].includes(response.status())) {
      await ensureAuthenticated(site, input.url, { force: true });
    }
    if (!response.ok()) {

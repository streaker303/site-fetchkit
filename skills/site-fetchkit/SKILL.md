...
`site-fetchkit fetch` 可以直接获取通用页面标题、正文、HTML 或简单 DOM 内容；已有更具体站点 skill 时，站点 skill 应通过 `site-fetchkit run`、登录态和 Runtime API 编排底层获取能力，并把业务解析留在站点 skill 内。外部脚本即使 `import "site-fetchkit"`，也必须用 `site-fetchkit run` 启动，不能直接用 `node <script>`。
...
`init` 只安装基础 skill 并准备状态目录，不自动安装 Chromium。`fetch` 默认使用 Playwright Chromium，`login` 也使用 Playwright Chromium 打开可见登录窗口；运行时报浏览器缺失时，再显式执行：
...
`fetch` 默认不启动系统 Google Chrome，避免后台抓取在沙箱环境触发系统 Chrome 异常退出弹窗。只有确实需要系统 Chrome 时，才显式执行：
...
4. 只有接口返回未授权、跳转到登录页、或站点 skill 判断登录态不可用时，才由 Agent 打开登录页面，不要求用户手动执行命令：
...
5. 用户回复“已登录”后，先执行确认命令保存登录态：

```bash
site-fetchkit confirm-login <site>
```

看到“登录态已保存”后，再重试原请求。`confirm-login` 会让仍在运行的登录流程保存 state，并默认用已保存 state 请求 `--url` 做通用校验；业务层是否真正登录成功、是否有页面权限，由具体站点 skill 判断。如果该站点无法用 HTTP 请求校验，可在启动登录时显式加 `--no-validate`，或用 `--validate-url <url>` 指定更适合校验的入口。

6. 如果用户目标是创建新站点能力，停止本 skill 流程，切换到 `site-fetchkit-site-creator`。

## 输出要求

- 根据调用路径输出抓取正文、站点 skill 的结构化结果或原始 JSON。
- 如果触发登录，说明登录页已打开，并停止当前抓取；用户回复“已登录”后执行 `site-fetchkit confirm-login <site>` 保存状态并重试。
...
- 登录窗口打开后，必须等待用户明确确认“已登录”再执行 `site-fetchkit confirm-login <site>` 保存登录态；用户确认前不主动保存登录态，也不主动关闭登录窗口。

## 维护说明

- 外部站点脚本必须通过 `site-fetchkit run ...` 执行；`run` 会预先挂载 Node ESM loader，让脚本内的 `import "site-fetchkit"` 解析到当前 CLI 包。
